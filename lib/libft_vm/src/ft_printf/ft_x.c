...
/*                                                 #+#   #+    #+    #+#      */
/*   Created: 2018/01/12 11:38:45 by fpupier      #+#   ##    ##    #+#       */
...
static int	ft_to_str(t_data *data)
{
	...
	{
		data->s_con = ft_strdup("0x");
		data->l_con = 2;
	}
	if (data->largeur && !data->argument)
		data->s_val = ft_strdup(" ");
	else if ((!data->diese && data->precision == 0 && !data->argument)
		|| (data->diese && data->precision == 0 && !data->argument))
		data->s_val = ft_strdup("");
	else
		data->s_val = ft_uitoa_base(data->argument, BASE_16_LOW);
